...
The current public Dataverse release does not require an API token. If access
rules change, set `DATAVERSE_API_TOKEN` in the shell or a server secret store.
...
By default, each case archive is retained while only `NIFTI/`,
`mask_pvp.nii.gz`, and `liver_mask_pvp.nii.gz` are extracted. Raw DICOM is not
needed for the planned experiments. Add `--extract-dicom` only if a later study
requires the original DICOM series, and increase the configured free-space
multiplier first.
