...
def validate_case(record: CaseRecord, load_intensities: bool = True) -> dict[str, Any]:
    ...
    for name, path in [
        ...
        mask_issue_severity = "error" if name == "tumor" else "warning"
        mask_result: dict[str, Any] = {"path": str(path), "exists": path.is_file()}
        masks[name] = mask_result
        if not path.is_file():
            issues.append(_issue(mask_issue_severity, f"missing_{name}_mask", str(path)))
            continue
        try:
            image = nib.load(str(path))
            ...
            issues.append(
                _issue(mask_issue_severity, f"unreadable_{name}_mask", str(exc))
            )
        ...
        if count:
            issues.append(
                _issue(
                    "warning",
                    "tumor_outside_liver",
                    f"{count} tumor voxels are outside the automatic liver pseudo-label",
                )
            )
