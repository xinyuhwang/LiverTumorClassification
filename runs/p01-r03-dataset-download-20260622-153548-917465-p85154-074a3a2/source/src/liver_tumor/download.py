...
class DataverseDownloader:
    ...
    def extract_case_archive(self, archive: Path) -> tuple[Path, str]:
        ...
        if (destination / "NIFTI" / "pvp.nii.gz").is_file() and (
            destination / "mask_pvp.nii.gz"
        ).is_file():
        ...
        try:
            with tarfile.open(archive, "r:*") as handle:
                self._safe_extract(handle, temporary)
            if not (temporary / "NIFTI" / "pvp.nii.gz").is_file():
                raise RuntimeError(f"Archive does not contain expected NIFTI/pvp.nii.gz: {archive}")
        ...
        for index, item in enumerate(selected, start=1):
            ...
            try:
                ...
                if extract and tarfile.is_tarfile(archive):
                    destination, status = self.extract_case_archive(archive)
