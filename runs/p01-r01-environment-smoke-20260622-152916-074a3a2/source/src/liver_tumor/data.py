...
def discover_cases(root: str | Path) -> list[CaseRecord]:
    ...
    for mask_path in sorted(root_path.glob(f"**/{TUMOR_MASK_FILENAME}")):
    ...
    for phase, path in record.phase_paths.items():
        ...
        try:
            ...
            if not np.allclose(image.affine, reference.affine, atol=1e-3, rtol=1e-5):
                issues.append(
                    _issue("error", f"affine_mismatch_{phase}", "Affine differs from PVP")
                )
            if load_intensities:
...
def audit_dataset(
    ...
    records = discover_cases(root_path)
    if max_cases:
        records = records[:max_cases]
    if not records:
        raise RuntimeError(f"No cases containing {TUMOR_MASK_FILENAME} found under {root_path}")
    ...
    summary = {
        ...
        "spacing": {
            axis: {
                "min": float(cases_frame[f"spacing_{axis}"].min()),
                "median": float(cases_frame[f"spacing_{axis}"].median()),
                "max": float(cases_frame[f"spacing_{axis}"].max()),
            }
            for axis in ["x", "y", "z"]
        },
        "tumor_volume_mm3": {
            "min": float(cases_frame["tumor_volume_mm3"].min()),
            "median": float(cases_frame["tumor_volume_mm3"].median()),
            "max": float(cases_frame["tumor_volume_mm3"].max()),
        },
    }
...
def create_split_manifest(
    ...
    if (frame.get("error_count", 0) > 0).any():
