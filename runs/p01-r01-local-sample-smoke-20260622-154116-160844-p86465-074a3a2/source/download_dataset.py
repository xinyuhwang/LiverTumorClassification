...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--extract-dicom", action="store_true")
...
def main() -> None:
    ...
    with RunContext(config, task=str(config["experiment"].get("task", "dataset-download"))) as run:
        ...
        inventory = downloader.run(
            ...
            extract_dicom=args.extract_dicom or bool(options.get("extract_dicom", False)),
        ...
        print(
            ...
            f"({inventory['dataset']['selected_bytes'] / 2**30:.2f} GiB; "
            f"estimated workspace "
            f"{inventory['dataset']['estimated_required_bytes'] / 2**30:.2f} GiB)"
        )
